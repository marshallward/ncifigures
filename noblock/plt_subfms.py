...
timings_files = {
    'block': 'mom6_fms_timings.yaml',
    'noblock': 'mom6_noblock_timings.yaml'
}
...
for fn_name in runtimes['block']:
    ...
    for model in models:
        ...
        for pt in rt:
            lines[model], = ax_rt.plot(pt[0], pt[1], 'o',
                            markeredgewidth=1, markersize=8, color=mark[model])
    ...
    ax_rt.legend((lines['block'], lines['noblock']),
                 ('Blocking', 'Nonblocking'), numpoints=1)
