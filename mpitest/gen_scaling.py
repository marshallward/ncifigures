...
for p in prof_data:
    pdata = prof_data[p]
    wdata = sorted([(rec['npes'], rec['runtimes']['total']) for rec in pdata])
...
xticks = [npe for npe in xticks if npe != 1440]
...
for ax in (ax_wt, ax_eff):
    for p in ('raijin', 'ht', '12ppn', 'ladder', 'snake'):
        ax.axvline(1920, color='k', linestyle=':', linewidth=0.5)
        try:
