...
for expt in timings:
    ...
    time = timings[expt]['runtime'][oname]
...
#fig.suptitle('Initialization scaling', fontsize=16, color='w')
...
for pt in ompi_runtimes:
    ompi_handle, = ax_rt.plot(pt[0], pt[1], 'x',
...
ax_rt.legend([orig_handle, ht_handle, ompi_handle],
             ['Original', 'Hyperthreaded', 'Latest'], loc='best')
...
for pt in ompi_runtimes:
    ax_eff.plot(pt[0], t_min * p_ref / pt[0] / pt[1], 'x',
                markeredgewidth=1, markersize=8, color='r')
