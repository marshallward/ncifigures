    ...
    for expt in timings[model]:
        for run in timings[model][expt]:
            ...
            for key in timings[model][expt][run]['runtimes']:
                ...
                # Attach MPI_Init to initialization
                if model in ('mom', 'mom6'):
                    ice_key = 'MPI_Init'

                    min_rt += timings[model][expt][run]['runtimes'][ice_key]['min']
                    mean_rt += timings[model][expt][run]['runtimes'][ice_key]['mean']
                    max_rt += timings[model][expt][run]['runtimes'][ice_key]['max']
                    std_rt += timings[model][expt][run]['runtimes'][ice_key]['std']
...
#            continue
#        min_rt = fms_timings[expt][run]['runtimes']['init']
#        mean_rt = fms_timings[expt][run]['runtimes']['init']
#        max_rt = fms_timings[expt][run]['runtimes']['init']
#        std_rt = fms_timings[expt][run]['runtimes']['init']
#        runtimes['mom6']['coupler_main_IP_coupler_init'].append((npes, mean_rt, min_rt, max_rt, std_rt))
#
...
#        max_rt = fms_timings[expt][run]['runtimes']['init']
#        std_rt = fms_timings[expt][run]['runtimes']['init']
#        runtimes['mom']['coupler_main_IP_coupler_init'].append((npes, mean_rt, min_rt, max_rt, std_rt))

fig, (ax_rt, ax_eff) = plt.subplots(1, 2, figsize=(12, 6))

fn_diff = {
    'mom': 'coupler_main_IP_coupler_init',
    'nemo': 'nemo_init',
    'mom6': 'coupler_main_IP_coupler_init',
}
...
ax_rt.legend((lines['nemo'], lines['mom'], lines['mom6']),
             ('NEMO 3.4', 'MOM 5.1', 'MOM 6'), numpoints=1, loc='best')

plt.savefig('mom_nemo_init.svg', facecolor='none', bbox_inches='tight')
plt.close()
