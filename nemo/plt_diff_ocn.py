...
for expt in fms_timings:
    for run in fms_timings[expt]:
        ...
        #    continue
        min_rt = fms_timings[expt][run]['runtimes']['ocn']
        mean_rt = fms_timings[expt][run]['runtimes']['ocn']
        max_rt = fms_timings[expt][run]['runtimes']['ocn']
        std_rt = fms_timings[expt][run]['runtimes']['ocn']
        runtimes['mom6']['update_ocean_model'].append((npes, mean_rt, min_rt, max_rt, std_rt))
...
for expt in fms_timings:
    for run in fms_timings[expt]:
        ...
        max_rt = fms_timings[expt][run]['runtimes']['ocn']
        std_rt = fms_timings[expt][run]['runtimes']['ocn']
        runtimes['mom']['update_ocean_model'].append((npes, mean_rt, min_rt, max_rt, std_rt))

fig, (ax_rt, ax_eff) = plt.subplots(1, 2, figsize=(12, 6))

fn_diff = {
    'mom': 'update_ocean_model',
    'nemo': 'stp',
    'mom6': 'update_ocean_model',
}
...
ax_rt.legend((lines['nemo'], lines['mom'], lines['mom6']),
             ('NEMO 3.4', 'MOM 5.1', 'MOM 6'), numpoints=1)

plt.savefig('mom_nemo_ocn.svg', facecolor='none', bbox_inches='tight')
plt.close()
