...
ax.set_xticklabels([pe for pe in runtimes])
...
for pe in runtimes:
    for pt in runtimes[pe]['ocn']:
        ocn_handler, = ax.plot(pe, pt, 'o', markeredgewidth=1, markersize=12, color='b')

for pe in runtimes:
    for pt in runtimes[pe]['ice']:
        ice_handler, = ax.plot(pe, pt, '^', markeredgewidth=1, markersize=8, color='r')
